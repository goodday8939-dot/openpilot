...
async function openToolsLanguageDialog() {
  ...
  const selected = await openAppDialog({
    mode: "choice",
    title: getUIText("language", "Language"),
    message: `${getUIText("current_language", "Current language")}: ${currentLabel}`,
    choiceLayout: "list",
  ...
  if (selected && selected !== LANG && typeof setWebLanguage === "function") {
    await Promise.resolve(setWebLanguage(selected));
  }
}
...
function renderToolsShortcuts() {
  const actionRoot = document.getElementById("toolsPrimaryActions");
  actionRoot?.setAttribute("aria-label", getUIText("tools_shortcuts", "Tools shortcuts"));
}
...
function rerenderPageLangUi() {
  ...
  if (CURRENT_PAGE === "logs") {
    ...
    globalThis.CarrotLogsRuntime?.screenrecord.render?.({ animate: false });
  }
...
function initToolsPage() {
  ...
  bindOnce("btnToolsCarSelect", () => {
    ...
    else if (typeof showPage === "function") showPage("car", true);
  });

  bindOnce("btnToolsLanguage", () => {
    openToolsLanguageDialog().catch((e) => showError("language", e));
  });

  bindOnce("btnToolsWebSettings", () => {
...
const CarrotToolsRuntime = Object.freeze({
  init: initToolsPage,
  refreshGitPullStatus,
  renderLanguage: rerenderPageLangUi,
  run: runTool,
  scheduleWarmup: scheduleUiWarmup,
  showError,
  logNotice: toolsLogNotice,
  getCommandPreview: getToolCommandPreview,
  renderMeta: renderToolsMeta,
});
